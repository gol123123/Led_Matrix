...
void MainWindow::on_cEnterText_returnPressed()
{
    ...
    Print_2(data); // Вывод данных в консоль
}
...
void MainWindow::Print_2(QString data)
{
    ui->consol_2->textCursor().insertText(data+'\r'); // Вывод текста в консоль
    ui->consol_2->moveCursor(QTextCursor::End);//Scroll
}
...
{
    char string[16] = "";
    QByteArray NumData("Ё");//// количиство байт
    const QByteArray data = QByteArray::fromHex("84");

    for(uint8_t i = 0; i <8; i++)
    {
        writeData(QByteArray::number(i+1)); //// отправляет номер строки

        if((DataLed[i][0]==0 || DataLed[i][1]==0) && !(DataLed[i][0]==0 && DataLed[i][1]==0))
        {
            if(DataLed[i][0]==0)
            { writeData(QByteArray::number(10)); }//// отправляет количество байт которое нужно прочесть
            else
            { writeData(QByteArray::number(11)); }
        }

        else if(DataLed[i][0]>0 && DataLed[i][1]>0)
        {
            writeData(QByteArray::number(20));//// отправляет количество байт которое нужно прочесть
        }
        ...
        for(uint8_t j = 0; j <2; j++)
        {
            ...
            itoa(DataLed[i][j], string, 16);
            writeData(string);
        }

    }

    writeData(QByteArray::number(84));
}
